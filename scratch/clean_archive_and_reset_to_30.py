...
archive_prefix = f"archive_session_best_15162usd_{timestamp}"

print(f"--- Archiving Current $15,162 session data to {BACKUP_DIR} ---")
...
# 2. Reset account_state.json to $30.00 starting balance
new_account_state = {
    "balance": 30.0,
    ...
    "pnl": 0.0,
    "total_pnl": 0.0,
    "last_change_reason": "clean-session-reset-30usd",
    "gas_balance": 5.0,
    "realized_pnl": 0.0
}
...
with open(DATA_DIR / "balance_history.jsonl", "w", encoding="utf-8") as f:
    f.write(json.dumps({"timestamp": datetime.utcnow().isoformat() + "Z", "balance": 30.0, "realized_pnl": 0.0}) + "\n")
...
print("--- Clean Slate Reset to $30.00 USDC completed successfully! ---")
