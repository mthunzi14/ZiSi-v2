...
ADDRESS = "0x8d1d5d1c6041b13fc708b5d9f668070e1724ed4a"
...
RESULTS_PATH = "/root/ZiSi-v2/data/certova_deep_analysis.txt"
...
def fetch_trades(max_trades=6000):
    print(f"[FETCH] Pulling up to {max_trades} trades for Certova...")
    trades = []
    seen = set()
    ...
    while len(trades) < max_trades and page < 80:
        page += 1
        params = {
            "user": ADDRESS,
            "type": "TRADE",
    ...
    print(f"  Completed! Total fetched Certova trades: {len(trades)}")
...
def analyze_trades(trades, cache):
    ...
    for slug, group in grouped.items():
        ...
        if buy_shares > 0:
            avg_entry_price = market_spent / buy_shares
        ...
        if winner:
            ...
            if is_win:
                ...
                if avg_entry_price > 0:
                    entry_prices_win.append(avg_entry_price)
            else:
                losses += 1
                ...
                if avg_entry_price > 0:
                    entry_prices_loss.append(avg_entry_price)
    ...
    avg_win_px = sum(entry_prices_win) / len(entry_prices_win) if entry_prices_win else 0.0
    avg_loss_px = sum(entry_prices_loss) / len(entry_prices_loss) if entry_prices_loss else 0.0
    
    # Write analysis file
    with open(RESULTS_PATH, "w", encoding="utf-8") as f:
        f.write("=========================================================\n")
        f.write("       CERTOVA WALLET DEEP STATISTICAL AUDIT REPORT\n")
        f.write("=========================================================\n\n")
        f.write(f"Address: {ADDRESS}\n")
        f.write(f"Total Trades Fetched: {len(trades)}\n")
        f.write(f"Resolved Markets: {resolved_markets}\n")
        f.write(f"Wins / Losses: {wins} / {losses}\n")
        f.write(f"Win Rate: {wr:.2f}%\n")
        f.write(f"Total Spent: ${spent:.2f}\n")
        f.write(f"Total Payout: ${payout:.2f}\n")
        f.write(f"Net P&L: ${pnl:.2f}\n")
        f.write(f"Profit Factor: {(payout/spent) if spent > 0 else 1.0:.2f}\n\n")
        
        f.write(f"Average Entry Price of Winning Markets: {avg_win_px*100:.1f}¢\n")
        f.write(f"Average Entry Price of Losing Markets:  {avg_loss_px*100:.1f}¢\n")
        f.write("---------------------------------------------------------\n\n")
        
        f.write("--- Timeframe Performance ---\n")
        for tf, s in sorted(tf_stats.items()):
            if s["count"] > 0:
                tf_wr = s["wins"] / s["count"] * 100
                tf_pnl = s["payout"] - s["spent"]
                f.write(f"  {tf:<6} | {s['count']:>4} markets | WR: {tf_wr:>5.1f}% | Net P&L: ${tf_pnl:>10.2f}\n")
        f.write("\n")
        
        f.write("--- Asset Performance ---\n")
        for asset, s in sorted(asset_stats.items()):
            if s["count"] > 0:
                as_wr = s["wins"] / s["count"] * 100
                as_pnl = s["payout"] - s["spent"]
                f.write(f"  {asset:<6} | {s['count']:>4} markets | WR: {as_wr:>5.1f}% | Net P&L: ${as_pnl:>10.2f}\n")
        f.write("\n")
        
        f.write("--- Session Performance ---\n")
        for sess, s in sorted(session_stats.items()):
            if s["count"] > 0:
                se_wr = s["wins"] / s["count"] * 100
                se_pnl = s["payout"] - s["spent"]
                f.write(f"  {sess:<20} | {s['count']:>4} markets | WR: {se_wr:>5.1f}% | Net P&L: ${se_pnl:>10.2f}\n")
                
    print(f"Deep Analysis Report generated and saved to {RESULTS_PATH}")

def main():
    cache = load_cache()
    print(f"Loaded {len(cache)} resolutions from cache.")
    
    trades = fetch_trades(6000)
