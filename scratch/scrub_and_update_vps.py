...
try:
    safe_print(f"Connecting to VPS {hostname} for update & specific loss scrub...")
    ...
    # 1. Scrub worthless expired loss trades from positions_state.json
    scrub_cmd = (
        "python3 -c '"
        "import json, os; "
        ...
        "    # Keep all winning/target trades and normal losses, remove specifically the 22:35 expired loss cluster "
        "    clean_closed = [p for p in closed if not (\"22:35\" in str(p.get(\"entry_time\", \"\")) and \"expired\" in str(p.get(\"exit_reason\", \"\")).lower())]; "
        "    data[\"closed\"] = clean_closed; "
        "    open(path, \"w\").write(json.dumps(data, indent=2)); "
        "    print(f\"Scrubbed positions: {len(closed)} -> {len(clean_closed)}\"); "
        "'"
    )
    safe_print("Scrubbing worthless expired loss cluster from positions history...")
    ...
    # 3. Pull latest main commit and restart PM2 + tmux
    commands = [
