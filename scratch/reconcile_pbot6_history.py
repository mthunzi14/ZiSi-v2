...
def main():
    history_path = "/root/ZiSi-v2/wallet/wallet_0x21d0a97a_history.json"
    ...
    # We will reconstruct closed positions by grouping buy transactions by Slug.
    # Note: TargetWallet is PBot-6.
    # For each unique slug, we find the net shares bought, average entry price, total spent, outcome resolved.
    ...
    for slug, actions in market_trades.items():
        ...
        # Outcome chosen (we assume the outcome of the first buy is the direction)
        direction = first_buy.get("BetOnOutcome", "Up")
        
        # Check resolution in cache
        # The cache has keys as slugs, values as winning outcome (e.g. "Up" or "Down" or "Yes" or "No")
        winning_outcome = winners.get(slug)
        if not winning_outcome:
            # try finding in other transactions if they have resolution
            unresolved_count += 1
            continue
            
        # Standardize outcomes
        # Winning outcome can be "Yes"/"No" or "Up"/"Down"
        # We need to map: "Yes" -> "Up", "No" -> "Down" (or vice versa depending on market outcome)
        # Let's inspect winning outcome values
        won = False
        # If winning outcome matches direction or equivalent
        ...
        if dt_str:
            try:
                # e.g. "2026-06-11T00:21:25+00:00"
                # remove timezone or parse with isoformat
    ...
    # Now analyze the gates of these resolved trades!
    print(f"\n==========================================")
    print(f"  P-BOT 6 GATES & PERFORMANCE RECONCILIATION")
    print(f"==========================================")
    ...
    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
        ...
        a_wins = sum(1 for x in group if x["won"])
        a_wr = (a_wins / len(group)) * 100
        print(f"  {day:10}: {len(group):3} trades | Win Rate: {a_wr:5.2f}% | PnL: ${a_pnl:8.2f}")
        
    # Save a report to a markdown file
    report_lines = [
        f"# 📊 PBot-6 Main Historical Gates & PnL Reconciliation",
        f"**Source File:** `wallet_0x21d0a97a_history.json`",
        f"**Resolved Winner Cache:** `resolved_winners_cache.json`",
        f"**Date Range:** Reconstructed from transaction logs.",
        f"",
    ...
    with open("/root/ZiSi-v2/wallet/wallet_0x21d0a97a_reconciliation.md", "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))
    print("\nSaved report to /root/ZiSi-v2/wallet/wallet_0x21d0a97a_reconciliation.md")
